...
const references = [
  ...
  { name: 'Марина, Одеса', image: maryna, text: 'Здаю квартиру в Одесі. Завжди було напружено: агенти, дзвінки, покази. Через Bokko сама призначаю зустріч, бачу профіль орендаря, можу відмовити. Це зручно та безпечно.' },
]
export function MarketingSections(props: Props) {
  ...
  if (part === 'reviews' && m.reference_reviews_enabled) return section('social_proof', <><h2 className="mk-gradient-title">{uk ? 'Відгуки · приклад із Bokko' : 'Reviews · Bokko reference example'}</h2><p className="mk-reference-note">{uk ? 'Відгуки про Bokko, відтворені як приклад оформлення. Це не відгуки про Natal.' : 'Reviews of Bokko reproduced as a design example. These are not reviews of Natal.'} <a href="https://www.bokko.in.ua/" target="_blank" rel="noopener noreferrer">Bokko ↗</a></p><div className="mk-reviews">{references.map(r => <blockquote key={r.name}><header><img src={r.image} alt="" /><div><strong>{r.name}</strong><span className="mk-stars" aria-label="5 / 5">{Array.from({ length: 5 }, (_, i) => <img key={i} src={star} alt="" />)}</span></div></header><p>{r.text}</p></blockquote>)}</div></>)
  ...
  if (part === 'footer' && m.footer_enabled) return <footer id={contactId} data-section="contacts" tabIndex={-1} className="mk-footer" onClickCapture={e => { if (editing) { e.preventDefault(); e.stopPropagation(); onSelect?.('contacts') } }}>
    <div><NatalMark /><div className="mk-legal">{copy.privacy_url && <a href={copy.privacy_url}>{uk ? 'Політика конфіденційності' : 'Privacy policy'}</a>}{copy.terms_url && <a href={copy.terms_url}>{uk ? 'Публічна оферта' : 'Terms of service'}</a>}</div></div>
