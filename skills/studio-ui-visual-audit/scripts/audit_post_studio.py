#!/usr/bin/env python3
"""Fail when representative Post Studio layouts clip, collide, or drift."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Optional directory for the exact audited 1080x1350 PNG variants.",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="ptw-studio-ui-audit-") as temporary:
        phone_workspace = PostStudioWorkspace(Path(temporary) / "phone")
