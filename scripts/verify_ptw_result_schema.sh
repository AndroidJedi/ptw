...
database_container="ptw-result-schema-$$"
cleanup() {
...
docker run --rm --detach --name "$database_container" \
  -e POSTGRES_DB=ptw_result_test \
  -e POSTGRES_USER=ptw_result_test \
  -e POSTGRES_PASSWORD=ptw-result-test-only \
  -v "$repository/db/migrations:/migrations:ro" \
  postgres:16-alpine >/dev/null

attempt=0
until docker exec "$database_container" pg_isready -U ptw_result_test -d ptw_result_test >/dev/null 2>&1; do
  attempt=$((attempt + 1))
...
apply_migrations() {
  docker exec "$database_container" sh -eu -c '
    args="-X -v ON_ERROR_STOP=1 -U ptw_result_test -d ptw_result_test"
...
actual=$(docker exec "$database_container" psql -X -qAt -U ptw_result_test -d ptw_result_test -c \
...
[ "$actual" = "$expected" ] || {
  echo "Result v1 schema differs from the exact table allowlist" >&2
  diff -u /dev/stdin /dev/stdin >/dev/null 2>&1 || true
  printf 'actual:\n%s\n' "$actual" >&2
  exit 1
}

docker exec "$database_container" psql -X -qAt -v ON_ERROR_STOP=1 \
  -U ptw_result_test -d ptw_result_test <<'SQL'
DO $$
BEGIN
  IF (SELECT count(*) FROM commander_schema_migrations) <> 1
    ...
    RAISE EXCEPTION 'the database must contain one Result v1 baseline migration';
  END IF;
...
echo "Verified the single clean Result v1 schema and idempotent migration journey."
