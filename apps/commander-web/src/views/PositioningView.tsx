...
export function PositioningView({ api }: { api: ApiClient }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      setProject(result.project); setRawIdea(''); setNotice('Research and strict synthesis started. No fallback will be used if either fails.'); await load()
  ...
  return <>
    ...
    {!projects.length ? <Empty><Target className="empty-mark" /><h2>No positioning yet</h2><p>Create the first project above. Production stays empty until you do.</p></Empty> : <div className="positioning-workspace">
      ...
      <div>{project && <section className="panel positioning-detail"><small>PROJECT {project.id}</small><h2>{project.raw_idea}</h2><p>{project.target_country} · research {project.research_language} · output {project.output_language}</p>
