...
export function ProjectSwitcher({ projects, projectId, onSelect, onNew, onRename }: {
  ...
  return <section className="project-switcher" aria-label="Project workspace">
    ...
    {selected && <p className="uuid-line">{selected.brief_count} Brief{selected.brief_count === 1 ? '' : 's'} · {selected.result_run_count} Result run{selected.result_run_count === 1 ? '' : 's'}</p>}
