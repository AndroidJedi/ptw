...
async function expectMonochromeChrome(page: Page) {
  const violations = await page.locator('body *').evaluateAll((elements) => {
    ...
    return elements.flatMap((element) => {
      if (element.closest('.studio-canvas')) return []
...
test('Ad Studio is a dedicated Project tab and saves a reusable template', async ({ page }) => {
  ...
  await expect(page.getByText('studio.placement.instagram.feed_square.v1', { exact: true }).first()).toBeVisible()
  const canvas = page.locator('.studio-canvas')
  await page.getByRole('button', { name: 'Edit', exact: true }).click()
  await page.getByRole('button', { name: 'studio.frame.headline.v1 frame' }).click()
  await expect(canvas).toBeInViewport()
  await expect(page.locator('.studio-inspector')).toBeInViewport()
  const headlineText = page.getByRole('textbox', { name: 'Text', exact: true })
  await expect(headlineText).toHaveValue('Meet a suitable psychologist with a calmer first step.')
  await headlineText.fill('A clearer editable headline')
  await expect(canvas.getByText('A clearer editable headline')).toBeVisible()
  await page.getByRole('button', { name: 'Close component inspector' }).click()
  await expect(page.locator('.studio-inspector')).toHaveCount(0)
  await expect(canvas).toBeInViewport()
  await page.getByRole('button', { name: /Save brand kit/ }).click()
  await page.getByPlaceholder('e.g. Cinematic clarity').fill('Reusable square')
  await page.getByRole('button', { name: /Save current template/ }).click()
  await expect(page.getByRole('status')).toContainText(`saved as ${studioTemplateId}`)
  await page.getByRole('button', { name: /Generate 5 editable posts/ }).click()
  const gallery = page.getByLabel('Five ready-to-share Studio posts')
