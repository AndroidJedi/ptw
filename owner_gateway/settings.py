...
class Settings:
    ...
    def from_environment(cls) -> "Settings":
        ...
        codex_token = os.environ.get("PTW_CODEX_AUTH_BRIDGE_TOKEN", "").strip()
        missing = [
            name for name, value in (
                ("FIREBASE_OWNER_UID", owner_uid),
                ("OWNER_GATEWAY_BRIDGE_TOKEN", token),
                ("PTW_CODEX_AUTH_BRIDGE_TOKEN", codex_token),
            ) if not value
        ]
        if missing:
        ...
        return cls(
            ...
            codex_authorization_service_url=os.environ.get("PTW_CODEX_AUTH_SERVICE_URL", "http://codex-auth:8094").rstrip("/"),
            codex_authorization_bridge_token=codex_token,
        )
