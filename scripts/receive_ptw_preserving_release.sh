...
if [[ $# -ne 4 ]]; then
    echo "usage: $0 RELEASE_TAG PTW_GIT_REVISION PLATFORM_GIT_REVISION DEPLOYED_PTW_REVISION" >&2
    exit 2
fi
release_tag=$1
git_revision=$2
platform_git_revision=$3
deployed_git_revision=$4
[[ $release_tag =~ ^[A-Za-z0-9._-]+$ && $release_tag != latest ]] || exit 2
[[ $git_revision =~ ^[0-9a-f]{40}$ && $platform_git_revision =~ ^[0-9a-f]{40}$ \
   && $deployed_git_revision =~ ^[0-9a-f]{40}$ ]] || exit 2
...
receive_artifact() {
    local expected_kind=$1 stream_name=$2 expected_image=${3:-}
    local header kind name blocks size digest artifact checksum actual architecture
    IFS= read -r header
...
receive_artifact IMAGE commander "ptw-commander:$release_tag"
receive_artifact IMAGE validation "ptw-validation:$release_tag"
receive_artifact IMAGE owner-gateway "ptw-owner-gateway:$release_tag"
receive_artifact IMAGE commander-god "ptw-commander-god:$release_tag"
receive_artifact IMAGE platform-commander-api "ptw-agent-platform-commander-api:$release_tag"
receive_artifact IMAGE platform-commander-worker "ptw-agent-platform-commander-worker:$release_tag"
receive_artifact IMAGE platform-codex-auth "ptw-agent-platform-codex-auth:$release_tag"
...
for component in commander validation owner-gateway commander-god; do
    ...
    [[ ($planned == 1 && $received == published) || ($planned == 0 && $received == reuse) ]] || {
...
elif [[ $platform_states == "published published published" ]]; then
