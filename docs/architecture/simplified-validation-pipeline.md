...
proposals. A separate pixel-aware creative validator runs automatically on
every initial Studio sample render and Wizard preview, can return a full
component recomposition, and allows at most three automatic recreations before
failing closed. Generated media is confined to an explicitly requested, reviewed,
...
`002_lesson_driven_creative_reruns.sql`, `003_validation_projects.sql`,
`004_ad_studio.sql`, and `005_ad_studio_creative_validation.sql` are
...
- Studio creative validation `evaluates` its exact proposal or render and
  `derived_from` the reviewed recipe, retaining every bounded review attempt.
...
validation modes and separately advertises `ad_studio_recipe_revision`,
`ad_studio_graphic_generation`, and `ad_studio_creative_validation`, without sharing Git history, database,
...
non-`latest` tag. This rollout uses additive `003`/`004`/`005` migrations and the
