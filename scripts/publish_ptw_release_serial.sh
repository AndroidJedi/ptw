...
if [[ $# -ne 2 ]]; then
    echo "usage: $0 RELEASE_TAG IMAGE_DIRECTORY" >&2
    exit 2
fi
...
[[ -z $(git status --porcelain --untracked-files=no) ]] || {
    echo "tracked local changes must be committed before production deployment" >&2
    exit 1
}
...
emit_image() {
    local name=$1
    local path=$2
    local size blocks padding digest
    ...
    if [[ $padding -gt 0 ]]; then
        padded=$(mktemp "/tmp/ptw-$name.XXXXXX.tar")
        cp "$path" "$padded"
        dd if=/dev/zero bs=1 count="$padding" >> "$padded" 2>/dev/null
        digest=$(sha256_file "$padded")
        printf 'IMAGE %s %s %s\n' "$name" "$blocks" "$digest"
        command cat "$padded"
        rm -f -- "$padded"
    else
...
emit_image idea-generation "$image_directory/idea-generation.tar" >> "$stream_file"
...
ssh -i "$HOME/.ssh/ptw_commander" -o IdentitiesOnly=yes root@165.245.212.184 \
    "set -e; exec 9>/run/lock/ptw-maintenance.lock; flock -n 9 || exit 73; git -C /root/ptw diff --quiet; git -C /root/ptw diff --cached --quiet; export PTW_MAINTENANCE_LOCK_HELD=1; git -C /root/ptw fetch origin '$revision'; git -C /root/ptw merge --ff-only '$revision'; exec /root/ptw/scripts/deploy_ptw_serial.sh '$release_tag' '$revision'" \
    < "$stream_file"
