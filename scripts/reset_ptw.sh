...
BEGIN
  ...
  FROM (VALUES
    ...
    ('studio_project_logo_defaults', (SELECT count(*) FROM studio_project_logo_defaults)),
  ...
  IF (SELECT count(*) FROM commander_schema_migrations) <> (SELECT count FROM expected_migration_inventory)
     ...
     ) OR NOT EXISTS (SELECT 1 FROM commander_schema_migrations WHERE name='012_phone_metrics_only.sql') THEN
