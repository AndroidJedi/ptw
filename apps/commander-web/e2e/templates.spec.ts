...
test('App Showcase opens before its preview completes and retains both versions after restart', async ({ page, backend }, info) => {
  ...
  await page.reload()
  await page.locator('.template-gallery .template-card').filter({ hasText: 'App Showcase' }).getByRole('button', { name: 'Open template' }).click()
  await expect(page.locator('.template-detail img')).toHaveCount(2, { timeout: 30000 })
  await page.getByLabel('Version').selectOption('1')
  await expect(page.getByLabel('Version')).toHaveValue('1')
  await expect(page.locator('.template-detail img')).toHaveCount(2, { timeout: 45000 })
})
