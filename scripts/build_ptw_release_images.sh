...
docker buildx build --platform linux/amd64 --load \
    --tag "ptw-idea-generation:$release_tag" \
    --file idea_generation/Dockerfile .
docker save --output "$output_directory/idea-generation.tar" "ptw-idea-generation:$release_tag"
...
if command -v sha256sum >/dev/null 2>&1; then
    sha256sum "$output_directory/commander.tar" \
        "$output_directory/idea-generation.tar" \
