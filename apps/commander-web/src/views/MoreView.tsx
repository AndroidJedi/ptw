...
interface SystemState { git_revision: string; services: Record<string, string>; emergency_stop: boolean; reset: { permitted: boolean } }
...
export function MoreView({ api }: { api: ApiClient }) {
  ...
  return <>
    ...
    {tab === 'system' && <section className="system-panel">
      ...
      <div className="service-list">{Object.entries(system?.services || {}).map(([name, status]) => <div key={name}><span>{name}</span><strong className={status === 'ok' ? 'ok' : 'bad'}>{status}</strong></div>)}</div>
      <div className="reset-gate"><small>НЕЗВОРОТНА РУЙНІВНА ДІЯ · без backup</small><input value={resetConfirmation} onChange={(event) => setResetConfirmation(event.target.value)} placeholder="RESET PTW PRODUCTION" /><button className="danger-outline" disabled={!system?.reset.permitted || resetConfirmation !== 'RESET PTW PRODUCTION'} onClick={async () => { setResetStatus('Виконується…'); try { await api.post('/api/v1/system/reset', { confirmation: resetConfirmation }); setResetStatus('Чисте скидання завершено') } catch (cause) { setResetStatus((cause as Error).message) } }}><RefreshCcw />Чисте скидання</button>{resetStatus && <p role="status">{resetStatus}</p>}</div>
    </section>}
