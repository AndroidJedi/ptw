...
function routeBaseUrl(path: string) {
  return localStudioMode && ['/api/v1/studio', '/api/v1/landings', '/api/v1/analytics', '/api/v1/instagram'].some(prefix => path.startsWith(prefix)) ? '' : baseUrl
}
...
export class ApiClient {
  ...
  private async headers(path: string, json = false): Promise<HeadersInit> {
    ...
    const localStudio = localStudioMode && (path.startsWith('/api/v1/studio') || path.startsWith('/api/v1/landings') || path.startsWith('/api/v1/instagram') || path.startsWith('/api/v1/analytics'))
