  ...
  return <section className="laval-engine">
    ...
    {showCreate && <div className="laval-create">
      ...
      <fieldset className="laval-mode"><legend>Режим доказів</legend>
        ...
        <label><input type="radio" name="evidence-mode" value="live" checked={requestedMode === 'live'} disabled={!providers?.search_live_ready} onChange={() => setRequestedMode('live')} /><span><strong>Живе дослідження</strong> — DataForSEO, максимум ${(providers?.max_spend_usd ?? .005).toFixed(3)}</span></label>
