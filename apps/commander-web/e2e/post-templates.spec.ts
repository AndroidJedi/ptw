...
test('apply accepted layout to an existing Post, edit, approve and restore after restart', async ({ page }, info) => {
  ...
  try {
    ...
    await page.getByRole('textbox', { name: 'title · Headline', exact: true }).fill('Owner edited headline')
    await page.getByText('Font and size for each text field', { exact: true }).click()
    await page.getByRole('slider', { name: 'title font size' }).fill('62')
    ...
    expect(before.configuration.template_typography.title.font_size).toBe(62)
    await stop(); await launch(); await page.reload()
    await page.getByText('Text and bullets', { exact: true }).click()
    await expect(page.getByRole('textbox', { name: 'title · Headline', exact: true })).toHaveValue('Owner edited headline')
    await page.getByText('Font and size for each text field', { exact: true }).click()
    await expect(page.getByRole('slider', { name: 'title font size' })).toHaveValue('62')
    await expect(page.locator('.phone-metrics-canvas-panel img')).toBeVisible()
