...
wget --quiet --mirror --page-requisites --adjust-extension --convert-links --no-parent --directory-prefix "$archive/site/mirror" "$origin/"
firebase hosting:sites:get natal-dashboard-dev --json > "$archive/metadata/firebase-site.json"
firebase hosting:channel:list --site natal-dashboard-dev --json > "$archive/metadata/firebase-channels.json"
git -C "$repository" rev-parse HEAD > "$archive/metadata/ptw-git-revision.txt"
date --utc --iso-8601=seconds > "$archive/metadata/archived-at.txt"
printf '%s\n' "$origin" > "$archive/metadata/source-origin.txt"

node "$repository/scripts/archive_natal_dashboard_screenshots.mjs" "$origin" "$archive/screenshots"
(
    cd "$archive"
