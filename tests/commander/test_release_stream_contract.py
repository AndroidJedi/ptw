...
class ReleaseStreamContractTests(unittest.TestCase):
    ...
    def test_fast_release_streams_only_planned_images_and_uses_selective_deployer(self) -> None:
        ...
        image_prune = receiver.index('docker image prune --force')
        builder_prune = receiver.index('docker builder prune --force --filter until=24h')
        receive_stream = receiver.index('IFS= read -r stream_version')
        self.assertLess(image_prune, receive_stream)
        self.assertLess(builder_prune, receive_stream)
        self.assertNotIn('docker volume prune', receiver)
        self.assertNotIn('docker image prune --all', receiver)
        ...
        self.assertIn('while sleep 20; do', deployer)
        self.assertIn('echo "PTW release canary is still running"', deployer)
        self.assertIn('run_with_progress_heartbeat "${validation_compose[@]}" run', deployer)
        self.assertIn('kill "$heartbeat_pid" 2>/dev/null || true', deployer)
        skill_repair = deployer.index('${PTW_TRUSTED_RELEASE_ROOT:-$repository}/scripts/install_ptw_skill_sync.sh')
        dependency_audit = deployer.index('audit_vps_owner_dependencies.sh')
        self.assertLess(skill_repair, dependency_audit)
    ...
    def test_mobile_release_keeps_coding_runtime_separate_from_privileged_transport(self) -> None:
        ...
        self.assertIn('REQUEST_PATH = ".ptw-release-request.json"', controller)
        self.assertIn('"push", "--atomic"', controller)
        self.assertIn('god-candidate/', controller)
        self.assertIn('PTW_TRUSTED_RELEASE_ROOT', receiver)
        plan_worker = compose.split("  commander-plan:", 1)[1].split("\nvolumes:\n", 1)[0]
        self.assertIn('/workspace:ro', plan_worker)
        self.assertNotIn('OWNER_GATEWAY_BRIDGE_TOKEN', plan_worker)
